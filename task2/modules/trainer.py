# This file is for GPT2 huggingface trainer
...
from transformers import Trainer, Seq2SeqTrainer, is_datasets_available
...
class GPT2Trainer(Seq2SeqTrainer):
    def __init__(self, *args, pad_token_id=None, post_process_function=None, **kwargs):
    ...
    def evaluate(
        ...
        if self.post_process_function is not None and self.compute_metrics is not None:
            ...
            eval_preds, eval_golds = self.post_process_function(eval_dataset, output.predictions)
            metrics = self.compute_metrics(eval_preds, eval_golds)
    ...
    def prediction_step(
        ...
        input_len = torch.sum(batch["attention_mask"])
        input_gen_len = torch.sum(inputs["labels"][0,:input_len].eq(-100))
        
        input_ids = inputs["input_ids"][:input_gen_len]
        attention_mask=inputs["attention_mask"][:input_gen_len]
        token_type_ids=inputs["token_type_ids"][:input_gen_len]
        ...
        labels = inputs["labels"][:, input_gen_len:input_len-1]
