...
class CQA(datasets.Metric):
    ...
    def _compute(self, predictions, references, no_answer_threshold=1.0):
        ...
        exact_raw, scores_raw = get_raw_scores(dataset, predictions)
        ...
        turned_eval = make_turn_based_eval_dict(exact_raw, f1_raw, precision_scores=precision_raw, recall_scores=recall_raw)
        ...
        precision_thresh = {id:scores_raw[id][1] for id in f1_thresh}
        recall_thresh = {id:scores_raw[id][2] for id in f1_thresh}

        out_eval = make_eval_dict(exact_thresh, f1_thresh, precision_scores=precision_thresh, recall_scores=recall_thresh)
        
        if has_ans_qids:
            has_ans_eval = make_eval_dict(exact_thresh, f1_thresh, qid_list=has_ans_qids, precision_scores=precision_thresh, recall_scores=recall_thresh)
        ...
        # merge_eval(out_eval, turned_eval, "Turns")
        out_eval['eval_Turns'] = turned_eval
